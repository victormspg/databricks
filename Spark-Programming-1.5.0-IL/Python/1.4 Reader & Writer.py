...
productsDF = spark.read.option("header", True).csv(productsCsvPath)
...
productsDF2 = spark.read.option("header", True).schema(userDefinedSchema).csv(productsCsvPath)
...
productsDF3 = spark.read.option("header", True).schema(DDLSchema).csv(productsCsvPath)
...
productsDF.write.mode("overwrite").format("delta").save(productsOutputPath)
