...
finalDF = (topTrafficDF
           .withColumn("avg_rev", expr("CAST((avg_rev * 100) AS LONG) / 100"))
           .withColumn("total_rev", expr("CAST((total_rev * 100) AS LONG) / 100"))
)
...
bonusDF = (topTrafficDF
           .withColumn("avg_rev", expr("ROUND(avg_rev, 2)"))
           .withColumn("total_rev", expr("ROUND(total_rev,2)"))
)
...
# TODO
chainDF = (df
           .groupBy("traffic_source").agg(sum("revenue").alias("total_rev"),avg("revenue").alias("avg_rev"))
           .sort(col("total_rev").desc())
           .limit(3)
           .withColumn("avg_rev", expr("ROUND(avg_rev, 2)"))
           .withColumn("total_rev", expr("ROUND(total_rev,2)"))
