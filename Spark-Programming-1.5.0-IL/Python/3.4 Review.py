...
schema = "firstName STRING, middleName STRING, lastName STRING, gender STRING, birthDate DATE, salary LONG, ssn STRING"

df = spark.read.schema(schema).option("header", True).option("sep", ":").csv(sourceFile)
df.count()

new_df = (df
          .withColumn("firstName_c", lower(col("firstName")))
          .withColumn("middleName_c", lower(col("middleName")))
          .withColumn("lastName_c", lower(col("lastName")))
          .withColumn("ssn_c", regexp_replace(col("ssn"), "-", ""))
          .dropDuplicates(["firstName_c", "middleName_c", "lastName_c", "gender", "birthDate", "salary", "ssn_c"])
          .drop("firstName_c", "middleName_c", "lastName_c", "ssn_c")
         )

new_df.repartition(8).write.mode("overwrite").parquet(destFile)
